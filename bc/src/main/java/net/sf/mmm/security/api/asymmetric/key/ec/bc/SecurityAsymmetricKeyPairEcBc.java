...
/**
 * An implementation of {@link SecurityAsymmetricKeyPair} for {@link BCECPrivateKey} and {@link BCECPublicKey}.
 *
 * @author Joerg Hohwiller (hohwille at users.sourceforge.net)
 * @since 1.0.0
 */
public class SecurityAsymmetricKeyPairEcBc extends SecurityAsymmetricKeyPairEc<BCECPrivateKey, BCECPublicKey> {
   ...
   */
  public SecurityAsymmetricKeyPairEcBc(BCECPrivateKey privateKey) {
   ...
   */
  public SecurityAsymmetricKeyPairEcBc(BCECPrivateKey privateKey, BCECPublicKey publicKey) {
  ...
  static KeyFactory getKeyFactory() {
    ...
    if (keyFactory == null) {
      try {
        keyFactory = KeyFactory.getInstance(SecurityAlgorithmEc.ALGORITHM_EC, BouncyCastle.getProvider());
  ...
  public static BCECPrivateKey createPrivateKey(ECPrivateKeySpec keySpec) {
    ...
    return new BCECPrivateKey(SecurityAlgorithmEc.ALGORITHM_EC, keySpec, BouncyCastleProvider.CONFIGURATION);
  }
  ...
  public static BCECPublicKey createPublicKey(ECPublicKeySpec keySpec) {
    ...
    return new BCECPublicKey(SecurityAlgorithmEc.ALGORITHM_EC, keySpec, BouncyCastleProvider.CONFIGURATION);
  }
