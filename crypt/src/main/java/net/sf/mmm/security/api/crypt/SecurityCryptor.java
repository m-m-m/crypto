 ...
 * {@link net.sf.mmm.security.api.key.asymmetric.SecurityAsymmetricKeyPair asymmetric} encryption.
 * Implementations are typically just wrappers of {@link javax.crypto.Cipher}. However this API gives additional
 * abstraction and flexibility. E.g. multiple {@link javax.crypto.Cipher}s can be combined without the need to change
 * the code using the {@link SecurityCryptor}.
 *
...
public abstract interface SecurityCryptor
  ...
  default byte[] process(byte[] input, int offset, int length) {
    ...
    return crypt(input, offset, length, true);
  }
